...
function CampaignPlan({ onBack, onStarted }) {
    ...
    const handleAnalyze = async () => {
        ...
        try {
            const res = await fetch(`${API_BASE}/api/campaigns/analyze`, {
                method: 'POST',
                headers: {
            ...
            const detailRes = await fetch(`${API_BASE}/api/campaigns/${data.campaign_id}`, {
                headers: { Authorization: `Bearer ${token}` },
            })
            if (detailRes.ok) {
    ...
    const handleRescan = async () => {
        ...
        try {
            const res = await fetch(`${API_BASE}/api/campaigns/${campaign.id}/rescan`, {
                method: 'POST',
                headers: { 
                    'Content-Type': 'application/json',
                    Authorization: `Bearer ${token}` 
            ...
            const detailRes = await fetch(`${API_BASE}/api/campaigns/${campaign.id}`, {
                headers: { Authorization: `Bearer ${token}` },
            })
            if (detailRes.ok) {
                const detail = await detailRes.json()
    ...
    const handleStart = async () => {
        ...
        try {
            const res = await fetch(`${API_BASE}/api/campaigns/${campaign.id}/start`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    Authorization: `Bearer ${token}`,
    ...
    const handleCancel = async () => {
        ...
        try {
            const res = await fetch(`${API_BASE}/api/campaigns/${campaign.id}/cancel`, {
                method: 'POST',
                headers: { Authorization: `Bearer ${token}` },
            })
            if (res.ok) {
