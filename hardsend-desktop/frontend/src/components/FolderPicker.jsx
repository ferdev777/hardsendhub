...
function FolderPicker({ isOpen, onClose, onSelect, mode = 'folder', fileExtension, title }) {
    ...
    // Load drives on first open or use Wails native dialog
    useEffect(() => {
        if (isOpen) {
            if (window.go?.main?.App?.SelectFolder) {
                // In Wails Desktop App, open OS native dialog
                const selectNative = async () => {
                    try {
                        const path = mode === 'folder'
                            ? await window.go.main.App.SelectFolder()
                            : await window.go.main.App.SelectTxtFile()
                        if (path) {
                            onSelect(path)
                        }
                    } catch (err) {
                        console.error('[FolderPicker] Error in Wails native dialog:', err)
                    } finally {
                        onClose()
                    }
                }
    ...
    const loadDrives = async () => {
        try {
            const res = await fetch(`${API_BASE}/api/filesystem/drives`, {
    ...
    const browse = async (path) => {
        ...
        try {
            let url = `${API_BASE}/api/filesystem/browse?path=${encodeURIComponent(path)}`
            if (mode === 'folder') {
                url += '&type=folder'
