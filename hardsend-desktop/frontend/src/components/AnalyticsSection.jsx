...
function AnalyticsSection({ token }) {
    ...
    const fetchAnalytics = async () => {
        ...
        try {
            const [resSummary, resTime] = await Promise.all([
                fetch(`${API_BASE}/api/analytics/summary?period=${range}`, {
                    headers: { Authorization: `Bearer ${token}` },
                }),
                fetch(`${API_BASE}/api/analytics/timeseries?period=${range}`, {
                    headers: { Authorization: `Bearer ${token}` },
                }),
            ])
            if (resSummary.ok) {
    ...
    const handleSyncResend = async () => {
        ...
        setSyncMessage('')
        try {
            const res = await fetch(`${API_BASE}/api/resend/sync`, {
                method: 'POST',
                headers: { Authorization: `Bearer ${token}` },
            })
            if (res.ok) {
                const data = await res.json()
                setSyncMessage(`✓ Sincronizado con Resend: ${data.total_fetched} consultados, ${data.updated_invoices} actualizados`)
                fetchAnalytics()
                setTimeout(() => setSyncMessage(''), 6000)
            } else {
                setSyncMessage('Error al sincronizar con Resend')
                setTimeout(() => setSyncMessage(''), 4000)
            }
        } catch (err) {
            console.error('[Analytics] Error syncing resend:', err)
            setSyncMessage('Error de conexión con Resend')
            setTimeout(() => setSyncMessage(''), 4000)
        } finally {
            setSyncing(false)
        }
    ...
    const CustomTooltip = ({ active, payload, label }) => {
        if (active && payload && payload.length) {
            return (
                <div className="glass-card p-3 text-xs border border-surface-600/50 shadow-xl bg-surface-900/95">
    ...
    return (
        <div className="glass-card p-6 animate-slide-up" style={{ animationDelay: '400ms' }}>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                ...
                <div className="flex flex-wrap items-center gap-2 self-start sm:self-auto">
                    <button
                        ...
                        disabled={syncing}
                        className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-medium bg-hardsend-500/20 hover:bg-hardsend-500/30 text-hardsend-300 border border-hardsend-500/30 transition-all duration-200 disabled:opacity-50"
                        title="Consultar API de Resend para actualizar estados de emails en tiempo real"
                    >
                        <RefreshCw className={`w-3.5 h-3.5 ${syncing ? 'animate-spin' : ''}`} />
                        <span>{syncing ? 'Sincronizando...' : 'Sincronizar con Resend'}</span>
                    </button>
                    <div className="flex items-center gap-1 bg-surface-800/80 p-1 rounded-lg border border-surface-700/50">
                        {ranges.map((r) => (
                            <button
                                key={r.id}
            ...
            {syncMessage && (
                <div className="mb-4 px-3 py-2 rounded-lg bg-surface-800/90 border border-hardsend-500/40 text-hardsend-300 text-xs font-medium animate-fade-in">
                    {syncMessage}
                </div>
            )}
